...
class VoronoiGraphGenerator:
    def __init__(self, cluster: str, env_name: str, floor: int):
        ...
        try:
            self._map = cv2.imread(os.path.join(os.path.dirname('src/neural_astar/utils/voronoi_utilities/'), 'maps_data', 'maps', cluster, env_name + '_floor_' + str(floor) + '.png'))
            with open(os.path.join(os.path.dirname('src/neural_astar/utils/voronoi_utilities/'), 'maps_data', 'maps_metadata', env_name + '_floor_' + str(floor)) + '.yaml', mode='r') as f:
    ...
    def to_numpy_array(self, st: List[Node]) -> np.array:
        array = np.full(self._map.shape, 0, dtype=int)
        for node in st:
